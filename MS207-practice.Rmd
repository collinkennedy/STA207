...
anova_model = aov(weight ~ Diet - 1, data = chickweight_data)
...
ggplot(data = chickweight_data, mapping = aes(x = anova_model$fitted.values, y = anova_resids))+
  geom_point(alpha = .8)+
...
summary(aov(d_ij ~ Diet, data = chickweight_data))

```

Constant variance assumption appears to be violated. It is difficult to say to what extent this is actually problematic though.

Now to consider the assumption of the normally distributed error term:
```{r}
ggplot(data = chickweight_data, mapping = aes(anova_resids))+
...
$$
where $\mu$ is the overall mean weight, $\alpha_i$ denotes the treatment effect of the $i^{th}$ diet on weight $i=1,...,4$, $\beta_j$ denotes the growth rate (aka, the effect of 'time') on `weight` (not sure what j is indexed up to), and $\epsilon_{ijk}$ is iid $N(0,\sigma^2)$ random error. ( think J is through 12)

Both parameters are subject to the constraint that
$$
\sum_{i = 1}^{4}\alpha_i = \sum_{j=1}^{idk} \beta_j = 0
$$
